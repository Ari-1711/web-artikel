...
import { Heart, Clock, Sparkles, Flame, Home, PenSquare, FileText } from "lucide-react";
...
function ArticleCard({
  ...
  if (featured) {
    return (
      <article
        className="card mb-4 p-4 border shadow-sm cursor-pointer bg-white"
        onClick={onOpenDetail}
        style={{ borderRadius: "12px" }}
      >
        <div className="d-flex flex-column gap-2">
          <span className="badge bg-danger bg-opacity-10 text-danger align-self-start px-2 py-1 text-uppercase" style={{ fontSize: "0.75rem", fontWeight: 600 }}>
            ★ Sorotan Utama
          </span>
          <h2 className="card-title h2 fw-bold text-dark my-1">{article.judul}</h2>
          <p className="card-text text-secondary" style={{ fontSize: "1rem", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.6" }}>
            {article.isi}
          </p>
          <div className="d-flex align-items-center justify-content-between pt-3 border-top mt-2">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                <p className="mb-0 fw-bold text-dark" style={{ fontSize: "0.875rem" }}>{namaPenulis}</p>
                <p className="mb-0 text-muted small">{formatDate(article.tanggalDibuat)}</p>
              </div>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); onLike(); }}
              className="btn btn-sm d-flex align-items-center gap-1 px-3 py-1.5 rounded-pill border-0"
              style={{
  ...
  return (
    <article
      className="card h-100 p-3 border shadow-sm cursor-pointer d-flex flex-column gap-2 bg-white"
      onClick={onOpenDetail}
      style={{ borderRadius: "12px" }}
    >
      <div className="flex-grow-1">
        <h3 className="h6 card-title fw-bold text-dark mb-2" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.4" }}>
          {article.judul}
        </h3>
        <p className="text-secondary small mb-0" style={{ display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.5" }}>
          {article.isi}
        </p>
      </div>
      <div className="d-flex align-items-center justify-content-between mt-auto pt-2 border-top">
        <div className="d-flex align-items-center gap-2 text-muted" style={{ fontSize: "0.75rem" }}>
          <span className="fw-semibold text-dark text-truncate" style={{ maxWidth: "90px" }}>{namaPenulis}</span>
          <span>·</span>
...
export function HomePage({
  ...
  if (loading) {
    return <div className="container py-5 text-center"><div className="spinner-border text-primary" role="status"></div><p className="mt-2 text-muted small">Memuat artikel...</p></div>;
  }
      ...
      {role === "admin" && (
        <div className="mt-5 d-flex align-items-center justify-content-between p-4 bg-light rounded border border-dashed border-primary">
          ...
          <div className="d-flex gap-2">
            <button onClick={() => setView("admin-dashboard")} className="btn btn-sm btn-outline-dark">
              Dashboard
            </button>
